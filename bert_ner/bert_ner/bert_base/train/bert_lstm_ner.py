...
os.environ["TF_CPP_MIN_LOG_LEVEL"]='0' # 这是默认的显示等级，显示所有信息  
# os.environ["TF_CPP_MIN_LOG_LEVEL"]='2' # 只显示 warning 和 Error   
# os.environ["TF_CPP_MIN_LOG_LEVEL"] = '3' # 只显示 Error
tf.logging.set_verbosity (tf.logging.INFO) # 启用tensorflow检测的输出
    ...
    def get_labels(self, labels=None):
        ...
        if os.path.exists(os.path.join(self.output_dir, 'label_list.pkl')):
            ...
            if len(self.labels) > 0:
                ...
                self.labels = ["O", "B-TIME", "I-TIME", "B-PERSON", "I-PERSON", "B-ORGANIZATION", "I-ORGANIZATION", "B-LOCATION", "I-LOCATION", "X", "[CLS]", "[SEP]"]
        return self.labels
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):
        ...
        print("使用参数构建模型,input_idx 就是输入的样本idx表示......")
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(label_ids, pred_ids):
                ...
                precision = tf_metrics.precision(label_ids, predictions, num_labels, [2, 3, 4, 5, 6, 7, 8, 9], average="macro")
                recall = tf_metrics.recall(label_ids, predictions, num_labels, [2, 3, 4, 5, 6, 7, 8, 9], average="macro")
                f = tf_metrics.f1(label_ids, predictions, num_labels, [2, 3, 4, 5, 6, 7, 8, 9], average="macro")
                # tf.scalar_summary('predictions', predictions)
                # tf.scalar_summary('eval_precision', precision)
                # tf.scalar_summary('eval_recall', recall)
                # tf.scalar_summary('eval_f', f)
                return {
...
def adam_filter(model_path):
    ...
    :return: 
    """
    print("去掉模型中的Adam相关参数，这些参数在测试的时候是没有用的")
...
def train(args):
    ...
    if args.do_train and args.do_eval:
        ...
        # 打印验证集数据信息
        print("2.Loding evaluation information......")
    ...
    if args.do_train and args.do_eval:
        ...
        print("5.train_and_evaluate")
        tf.estimator.train_and_evaluate(estimator, train_spec, eval_spec)
        print("6.train_and_evaluate over")
    ...
    if args.do_predict:
        ...
        print("  Num examples = %d", len(predict_examples))
        print("  Batch size = %d", args.batch_size)
    ...
    if args.filter_adam_var:
        print("7.Output result into file......")
